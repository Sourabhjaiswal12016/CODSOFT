...
class Movie{
	protected:
		...
		int duration;
		double ticketPrice;
		bool seats[50];//an array to check the availibility of seats:
		
		
	public:Movie(string t,string rd,int d,double tp): title(t),releaseDate(rd),duration(d),ticketPrice(tp){
		for(int i=0;i<50;i++){
...
int main()
{
	const string adminPassword="sourabh";
	...
	do{
        ...
        switch (choice){
			...
			case 4://cinema me kaun si movie chal rahi hai :
				{
...
//				
	  ...
	  while(choice!=7);//silently says that when ever it becomes 7 just return;
